...
class PusherEnv(MujocoEnv, utils.EzPickle):
    ...
    * v0: Initial versions release (1.0.0)
    ...
    def __init__(self, object_random=False, **kwargs):
        ...
        print(f"Loading XML file: {absolute_path}")  # Print the absolute path of the XML file

        MujocoEnv.__init__(self, absolute_path, 5)
    ...
    def _check_contact(self, geom1, geom2):
        for contact in self.data.contact[: self.data.ncon]:
            if (self.model.geom_id2name(contact.geom1) == geom1 and self.model.geom_id2name(contact.geom2) == geom2) or \
                    (self.model.geom_id2name(contact.geom1) == geom2 and self.model.geom_id2name(
                        contact.geom2) == geom1):
                return True
        return False
    ...
    def _get_obs(self):
        return np.concatenate(
            [
                ...
                self.get_body_com("obstacle1"),
                self.get_body_com("obstacle2"),
                self.get_body_com("obstacle3"),
            ]
        )
